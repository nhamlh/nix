...
with builtins;
with lib;
let cfg = config.my.modules.services.cloudflare-warp;
in {
  options.my.modules.services.cloudflare-warp = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    networking.firewall =
      lib.mkIf cfg.openFirewall { allowedUDPPorts = [ cfg.udpPort ]; };

    systemd.tmpfiles.rules =
      [ "d ${cfg.rootDir}    - root root" "z ${cfg.rootDir}    - root root" ];
    ...
    systemd.services.cloudflare-warp = {
      ...
      serviceConfig =
        let caps = [ "CAP_NET_ADMIN" "CAP_NET_BIND_SERVICE" "CAP_SYS_PTRACE" ];
        in {
          Type = "simple";
